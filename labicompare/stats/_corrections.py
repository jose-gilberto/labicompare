...
class HolmCorrection(Correction):
...
class SimesHochbergCorrection(Correction):
	...
	def apply(p_values, alpha = 0.05, max_iter = 1, is_sorted = False, return_sorted = False):
		...
		alpha_sh = alpha_f / np.arange(n_tests, 0, -1)
		reject = p_values <= alpha_sh
		reject_idx = np.nonzero(reject)

		if reject_idx[0].size > 0:
			reject_max = np.max(np.nonzero(reject))
			reject[:reject_max] = True

		p_values_corrected_raw = np.arange(n_tests, 0, -1) * p_values
		p_values_corrected = np.minimum.accumulate(p_values_corrected_raw[::-1])[::-1]
		del p_values_corrected_raw
...
class Hommel(Correction):
	...
	def apply(p_values, alpha = 0.05, max_iter = 1, is_sorted = False, return_sorted = False):
		...
		for m in range(n_tests, 1, -1):
			cim = np.min(m * p_values[-m:] / np.arange(1, m + 1.))
			p_values_copy[-m:] = np.maximum(a[-m:], cim)
			p_values_copy[:-m] = np.maximum(a[:-m], np.minimum(m * p_values[:-m], cim))

		p_values_corrected = p_values_copy
